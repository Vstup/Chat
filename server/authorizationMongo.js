    ...
    db.chatdb.collection('users').findOne({username:uname},function(err, result) {
        ...
        if (result){
          res.end('There is an user with such a name');
            ...
            db.chatdb.collection('users').insertOne({username:uname,pass:passwd,email:email},function(err, result) {
                if (err) throw err;
                callback();
            });
          }
...
const checkPass = (uname, passwd,callback) => {
    db.chatdb.collection('users').findOne({username: uname,pass:passwd},function (err, result) {
        if (err) throw err;
...
const newUserSession = (uname,callback) => {
    db.chatdb.collection('sessions').insertOne({
        sessionId:token.generateSessId,
        active:false,
        uname:uname,
        token:''
    },function(err, result) {
